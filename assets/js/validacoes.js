 ...
 * Funções puras de validação + helpers de UI para mostrar erro por campo.
 */
...
function validarNome(nome) {
  ...
  if (!/^[A-Za-zÀ-ÖØ-öø-ÿ\s']+$/.test(valor)) return "O nome deve conter apenas letras e espaços.";
  if (valor.trim().split(/\s+/).length < 2) return "Digite o nome completo (nome e sobrenome).";
  return null;
}
...
function validarEmail(email) {
  ...
  const regex = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;
  if (!regex.test(valor)) return "Digite um e-mail válido.";
  return null;
}
...
function validarTelefone(telefone) {
  ...
  if (digitos.length < 10 || digitos.length > 11) return "Telefone inválido. Use DDD + número.";
  return null;
}
...
function validarCPF(cpf) {
  ...
  if (resto !== parseInt(digitos[9])) return "CPF inválido.";

  soma = 0;
  for (let i = 0; i < 10; i++) soma += parseInt(digitos[i]) * (11 - i);
  resto = (soma * 10) % 11;
  if (resto === 10 || resto === 11) resto = 0;
  if (resto !== parseInt(digitos[10])) return "CPF inválido.";
...
function validarDataNascimento(data) {
  ...
  if (nascimento > hoje) return "A data de nascimento não pode ser no futuro.";
...
function validarCampoObrigatorio(valor, nomeCampo) {
  if (!valor || !valor.trim()) return `${nomeCampo} é obrigatório.`;
  return null;
}
