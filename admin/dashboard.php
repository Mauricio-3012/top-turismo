...
<?php foreach ($destinos as $destino): ?><tr><td><img src="../<?= htmlspecialchars($destino['img_destino']) ?>" alt="<?= htmlspecialchars($destino['nome_destino']) ?>"></td><td><strong><?= htmlspecialchars($destino['nome_destino']) ?></strong></td><td><?= htmlspecialchars($destino['cidade_destino'] . ' - ' . ($destino['estado_destino'] ?? '')) ?></td><td>R$ <?= number_format((float)$destino['preco_destino'],2,',','.') ?></td><td><i class="bi bi-star-fill text-warning"></i> <?= number_format((float)($destino['avaliacao_destino'] ?? 0),1,',','.') ?></td><td><div class="admin-actions justify-content-end"><a class="btn btn-sm btn-outline-primary" href="editar-destino.php?id=<?= (int)$destino['id_destino'] ?>"><i class="bi bi-pencil"></i> Editar</a><form method="post" action="../php/admin/excluir-destino.php" onsubmit="return confirm('Excluir este destino? Esta ação não pode ser desfeita.');"><input type="hidden" name="id_destino" value="<?= (int)$destino['id_destino'] ?>"><button class="btn btn-sm btn-outline-danger" type="submit"><i class="bi bi-trash"></i> Excluir</button></form></div></td></tr><?php endforeach; ?>
